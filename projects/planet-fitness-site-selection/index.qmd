...
```{=html}
<div class="pf-gate" id="pf-gate">
  <h4>🔒 Want the exact addresses and the full deck?</h4>
  <p>Enter your email to unlock the specific recommended site addresses below, and to download the full presentation (PDF &amp; PPTX).</p>
  <form class="pf-gate-form" id="pf-gate-form">
    <input type="email" id="pf-gate-email" placeholder="you@company.com" required aria-label="Email address">
    <button type="submit">Unlock</button>
  </form>
  <div class="pf-gate-status" id="pf-gate-status"></div>
</div>
```
...
<figure class="figure">
<p><img src="other_docs/2_La_Mesa.jpg" class="img-fluid figure-img pf-locked-map" alt="La Mesa recommended sites overlaid on the demographic heat map."></p>
<figcaption>La Mesa recommended sites overlaid on the demographic heat map.</figcaption>
</figure>
<div class="pf-map-lock-badge">🔒 Site list on this map is hidden — unlock below to reveal</div>
</div>
```
...
**Top candidate sites:** <span class="pf-locked-address">7928 El Cajon Blvd</span> (score 9.1, 4.19 acres, $67,500 income) · <span class="pf-locked-address">7900 La Mesa Blvd</span> (score 9.1, 3.53 acres, $67,500 income)
...
<div class="pf-map-wrap">
<figure class="figure">
<p><img src="other_docs/3_Oceanside.jpg" class="img-fluid figure-img pf-locked-map" alt="Oceanside recommended sites overlaid on the demographic heat map."></p>
<figcaption>Oceanside recommended sites overlaid on the demographic heat map.</figcaption>
</figure>
<div class="pf-map-lock-badge">🔒 Site list on this map is hidden — unlock below to reveal</div>
</div>
```
...
**Top candidate sites:** <span class="pf-locked-address">2005 Mission Ave</span> (score 9.1, 9.52 acres, $71,371 income) · <span class="pf-locked-address">1006 Mission Ave</span> (score 9.1, 9.52 acres, $72,750 income) · <span class="pf-locked-address">3308 Mission Ave</span> (score 9.1, 2.53 acres, $51,607 income)
...
<div class="pf-map-wrap">
<figure class="figure">
<p><img src="other_docs/4_Escondido.jpg" class="img-fluid figure-img pf-locked-map" alt="Escondido recommended sites overlaid on the demographic heat map."></p>
<figcaption>Escondido recommended sites overlaid on the demographic heat map.</figcaption>
</figure>
<div class="pf-map-lock-badge">🔒 Site list on this map is hidden — unlock below to reveal</div>
</div>
```
...
**Top candidate sites:** <span class="pf-locked-address">2040 Valley Pkwy</span> (score 9.1, 1.92 acres, $56,230 income) · <span class="pf-locked-address">2315 Valley Pkwy</span> (score 9.1, 2.54 acres, $56,230 income) · <span class="pf-locked-address">2427 Valley Pkwy</span> (score 9.1, 2.22 acres, $73,693 income)
:::
...
::::: {.pf-gated-downloads .pf-locked #pf-gated-downloads}
::: {.pf-downloads}
[Download the full presentation (PDF)](other_docs/PlanetFitness_SiteSelection_Presentation.pdf){.pf-download-btn target="_blank" onclick="return window.pfGateUnlocked;"} [Download the original slides (.pptx)](other_docs/PlanetFitness_SiteSelection_Presentation.pptx){.pf-download-btn .pf-download-btn-outline onclick="return window.pfGateUnlocked;"}
:::
<p class="pf-gated-lock-msg" id="pf-gated-lock-msg">🔒 Enter your email above to unlock downloads.</p>
:::::
...
<script>
(function () {
  window.pfGateUnlocked = false;
  var GATE_KEY = 'pf_gate_unlocked_planet_fitness';
  var form = document.getElementById('pf-gate-form');
  var statusEl = document.getElementById('pf-gate-status');
  var gateBox = document.getElementById('pf-gate');
  var downloadsWrap = document.getElementById('pf-gated-downloads');
  var lockMsg = document.getElementById('pf-gated-lock-msg');

  function unlock() {
    window.pfGateUnlocked = true;
    document.querySelectorAll('.pf-locked-address').forEach(function (el) {
      el.classList.add('pf-unlocked');
    });
    document.querySelectorAll('.pf-locked-map').forEach(function (el) {
      el.classList.add('pf-unlocked');
    });
    document.querySelectorAll('.pf-map-wrap').forEach(function (el) {
      el.classList.add('pf-unlocked');
    });
    if (downloadsWrap) downloadsWrap.classList.remove('pf-locked');
    if (lockMsg) lockMsg.style.display = 'none';
    if (gateBox) {
      var h = gateBox.querySelector('h4');
      var p = gateBox.querySelector('p');
      if (h) h.textContent = '✅ Unlocked — thanks!';
      if (p) p.textContent = 'The exact addresses are visible below and downloads are enabled.';
  ...
  if (form) {
    form.addEventListener('submit', function (e) {
      e.preventDefault();
      var emailInput = document.getElementById('pf-gate-email');
      var email = emailInput.value.trim();
      if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
        statusEl.textContent = 'Please enter a valid email address.';
        statusEl.className = 'pf-gate-status pf-error';
        return;
      }
      statusEl.textContent = 'Unlocking…';
      statusEl.className = 'pf-gate-status';
      fetch('https://formsubmit.co/ajax/mariononog@gmail.com', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
        body: JSON.stringify({
          email: email,
          _subject: 'Planet Fitness project page — download/address unlock',
        ...
        // Don't block the visitor even if the notification email fails to send.
        localStorage.setItem(GATE_KEY, 'true');
        unlock();
      });
    });
